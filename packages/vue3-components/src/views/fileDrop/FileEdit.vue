<template>
  <div id="fileBodyBox" class="file-box" @paste="onPasted">
    ...
    <el-dialog
      v-model="dialogVisible" width="598px" @opened="dialogOpened" @closed="dialogClosed"
...
// import { httpsFn } from 'apis-ts';
import { ConfigProvider } from 'ant-design-vue';
// import { AFileEntity } from '../entitys/IFileEntity';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function getValue(): any {
      // let isUpdate = true;
      // for (let x of value.value) {
      //   let res = widgetState.fileList.findIndex(y => x.id !== y.uid) > -1;
      //   if (res) {
      //     isUpdate = false;
      //     continue;
      //   }
      // }
      // return isUpdate ? [...value.value] : value.value;
      return [...value.value];
    }
    ...
    */
     ...
     * 文件上传成功后回调
     */
    function onFileSuccessed(_result: any) {
      if (_result.code === 200) {
    ...
    */
    ...
    async function handlePreview(file: any) {
      if (file.type.toLocaleLowerCase().indexOf('image') > -1 && file.url) {
        widgetState.urls = [file.url];
        widgetState.imgViewrVisible = true;
      } else {
        // var elemIF = document.createElement('iframe');
        // elemIF.src = process.env.VUE_APP_API_PATH + file.url;
        // elemIF.style.display = 'none';
        // console.log("文件地址", process.env.VUE_APP_API_PATH + file.url);
        // downloadFile()
    ...
    function fileClicked(file: any) {
      if (file.type.toLocaleLowerCase().indexOf('image') > -1 && file.url) {
        widgetState.urls = [process.env + file.url];
        widgetState.imgViewrVisible = true;
      } else {
        ElMessage.info(`开始后台下载文件: ${file.name},请勿刷新页面.`);
